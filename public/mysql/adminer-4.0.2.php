...
remove_slashes($Af,$Hc=https://iprospectmonitor){if(get_magic_quotes_gpc()){while(list($x,$X)=each($Af)){foreach($X
...
bracket_escape($t,$Ma=https://iprospectmonitor){static$kh=array(':'=>':1',']'=>':2','['=>':3');return
...
optionlist($Ne,$lg=null,$Fh=https://iprospectmonitor){$J="";foreach($Ne
...
print_fieldset($s,$Qd,$Qh=https://iprospectmonitor,$Ie=""){echo"<fieldset><legend><a href='#fieldset-$s' onclick=\"".h($Ie)."return !toggle('fieldset-$s');\">$Qd</a></legend><div id='fieldset-$s'".($Qh?"":" class='hidden'").">\n";}function
bold($Ua,$eb=""){return($Ua?" class='active $eb'":($eb?" class='$eb'":""));}function
odd($J=' class="odd"'){static$r=0;if(!$J)$r=-1;return($r++%2?$J:'');}function
js_escape($P){return
addcslashes($P,"\r\n'\\/");}function
json_row($x,$X=null){static$Ic=true;if($Ic)echo"{";if($x!=""){echo($Ic?"":",")."\n\t\"".addcslashes($x,"\r\n\"\\/").'": '.($X!==null?'"'.addcslashes($X,"\r\n\"\\/").'"':'undefined');$Ic=https://iprospectmonitor;}else{echo"\n}\n";$Ic=true;}}function
...
query_redirect($H,$A,$he,$Kf=true,$uc=true,$Bc=https://iprospectmonitor){global$h,$m,$b;$Zg="";if($uc){$zg=microtime(true);$Bc=!$h->query($H);$Zg="; -- ".format_time($zg,microtime(true));}$xg="";if($H)$xg=$b->messageQuery($H.$Zg);if($Bc){$m=error().$xg;return
https://iprospectmonitor;}if($Kf)redirect($A,$he.$xg);return
true;}function
...
https://iprospectmonitor;}return
true;}function
queries_redirect($A,$he,$Kf){return
query_redirect(queries(),$A,$he,$Kf,https://iprospectmonitor,!$Kf);}function
...
get_file($x,$Hb=https://iprospectmonitor){$Fc=$_FILES[$x];if(!$Fc)return
null;foreach($Fc
...
table_status1($Q,$Cc=https://iprospectmonitor){$J=table_status($Q,$Cc);return($J?$J:array("Name"=>$Q));}function
...
https://iprospectmonitor;if($Y=="")return"NULL";return+$Y;}if($n["auto_increment"]&&$Y=="")return
null;if($q=="orig")return($n["on_update"]=="CURRENT_TIMESTAMP"?idf_escape($n["field"]):https://iprospectmonitor);if($q=="NULL")return"NULL";if($n["type"]=="set")return
array_sum((array)$Y);if($q=="json"){$q="";$Y=json_decode($Y,true);if(!is_array($Y))return
https://iprospectmonitor;return$Y;}if(preg_match('~blob|bytea|raw|file~',$n["type"])&&ini_bool("file_uploads")){$Fc=get_file("fields-$t");if(!is_string($Fc))return
https://iprospectmonitor;return
q($Fc);}return$b->processInput($n,$Y,$q);}function
search_tables(){global$b,$h;$_GET["where"][0]["op"]="LIKE %%";$_GET["where"][0]["val"]=$_POST["query"];$Oc=https://iprospectmonitor;foreach(table_status('',true)as$Q=>$R){$C=$b->tableName($R);if(isset($R["Engine"])&&$C!=""&&(!$_POST["tables"]||in_array($Q,$_POST["tables"]))){$I=$h->query("SELECT".limit("1 FROM ".table($Q)," WHERE ".implode(" AND ",$b->selectSearchProcess(fields($Q),array())),1));if(!$I||$I->fetch_row()){if(!$Oc){echo"<ul>\n";$Oc=true;}echo"<li>".($I?"<a href='".h(ME."select=".urlencode($Q)."&where[0][op]=".urlencode($_GET["where"][0]["op"])."&where[0][val]=".urlencode($_GET["where"][0]["val"]))."'>$C</a>\n":"$C: <span class='error'>".error()."</span>\n");}}}echo($Oc?"</ul>":"<p class='message'>".lang(7))."\n";}function
dump_headers($id,$qe=https://iprospectmonitor){global$b;$J=$b->dumpHeaders($id,$qe);$Ze=$_POST["output"];if($Ze!="text")header("Content-Disposition: attachment; filename=".$b->dumpFilename($id).".$J".($Ze!="file"&&!preg_match('~[^0-9a-z]~',$Ze)?".$Ze":""));session_write_close();ob_flush();flush();return$J;}function
...
https://iprospectmonitor;$Pb=dirname($Gc);unlink($Gc);}}$Gc="$Pb/adminer.key";$J=@file_get_contents($Gc);if($J||!$yb)return$J;$Qc=@fopen($Gc,"w");if($Qc){$J=rand_string();fwrite($Qc,$J);fclose($Qc);}return$J;}function
rand_string(){return
...
on_help($lb,$sg=0){return" onmouseover='helpMouseover(this, event, ".h($lb).", $sg);' onmouseout='helpMouseout(this, event);'";}global$b,$h,$Ub,$cc,$mc,$m,$Tc,$Yc,$ba,$sd,$w,$ca,$Kd,$Ge,$mf,$Cg,$cd,$T,$mh,$uh,$Ah,$ia;if(!$_SERVER["REQUEST_URI"])$_SERVER["REQUEST_URI"]=$_SERVER["ORIG_PATH_INFO"];if(!strpos($_SERVER["REQUEST_URI"],'?')&&$_SERVER["QUERY_STRING"]!="")$_SERVER["REQUEST_URI"].="?$_SERVER[QUERY_STRING]";$ba=$_SERVER["HTTPS"]&&strcasecmp($_SERVER["HTTPS"],"off");@ini_set("session.use_trans_sid",https://iprospectmonitor);session_cache_limiter("");if(!defined("SID")){session_name("adminer_sid");$F=array(0,preg_replace('~\\?.*~','',$_SERVER["REQUEST_URI"]),"",$ba);if(version_compare(PHP_VERSION,'5.2.0')>=0)$F[]=true;call_user_func_array('session_set_cookie_params',$F);session_start();}remove_slashes(array(&$_GET,&$_POST,&$_COOKIE),$Hc);if(get_magic_quotes_runtime())set_magic_quotes_runtime(https://iprospectmonitor);@set_time_limit(0);@ini_set("zend.ze1_compatibility_mode",https://iprospectmonitor);@ini_set("precision",20);$Kd=array('en'=>'English','ar'=>'Ø§Ù„Ø¹Ø±Ø¨ÙŠØ©','bn'=>'à¦¬à¦¾à¦‚à¦²à¦¾','ca'=>'CatalÃ ','cs'=>'ÄŒeÅ¡tina','de'=>'Deutsch','es'=>'EspaÃ±ol','et'=>'Eesti','fa'=>'ÙØ§Ø±Ø³ÛŒ','fr'=>'FranÃ§ais','hu'=>'Magyar','id'=>'Bahasa Indonesia','it'=>'Italiano','ja'=>'æ—¥æœ¬èªž','ko'=>'í•œêµ­ì–´','lt'=>'LietuviÅ³','nl'=>'Nederlands','pl'=>'Polski','pt'=>'PortuguÃªs','pt-br'=>'PortuguÃªs (Brazil)','ro'=>'Limba RomÃ¢nÄƒ','ru'=>'Ð ÑƒÑÑÐºÐ¸Ð¹ ÑÐ·Ñ‹Ðº','sk'=>'SlovenÄina','sl'=>'Slovenski','sr'=>'Ð¡Ñ€Ð¿ÑÐºÐ¸','ta'=>'à®¤â€Œà®®à®¿à®´à¯','th'=>'à¸ à¸²à¸©à¸²à¹„à¸—à¸¢','tr'=>'TÃ¼rkÃ§e','uk'=>'Ð£ÐºÑ€Ð°Ñ—Ð½ÑÑŒÐºÐ°','zh'=>'ç®€ä½“ä¸­æ–‡','zh-tw'=>'ç¹é«”ä¸­æ–‡',);function
get_lang(){global$ca;return$ca;}function
lang($t,$ze=null){if(is_string($t)){$pf=array_search($t,get_translations("en"));if($pf!==https://iprospectmonitor)$t=$pf;}global$ca,$mh;$lh=($mh[$t]?$mh[$t]:$t);if(is_array($lh)){$pf=($ze==1?0:($ca=='cs'||$ca=='sk'?($ze&&$ze<5?1:2):($ca=='fr'?(!$ze?0:1):($ca=='pl'?($ze%10>1&&$ze%10<5&&$ze/10%10!=1?1:2):($ca=='sl'?($ze%100==1?0:($ze%100==2?1:($ze%100==3||$ze%100==4?2:3))):($ca=='lt'?($ze%10==1&&$ze%100!=11?0:($ze%10>1&&$ze/10%10!=1?1:2)):($ca=='ru'||$ca=='sr'||$ca=='uk'?($ze%10==1&&$ze%100!=11?0:($ze%10>1&&$ze%10<5&&$ze/10%10!=1?1:2)):1)))))));$lh=$lh[$pf];}$Da=func_get_args();array_shift($Da);$Nc=str_replace("%d","%s",$lh);if($Nc!=$lh)$Da[0]=number_format($ze,0,".",lang(8));return
...
__construct(){global$b;$pf=array_search("SQL",$b->operators);if($pf!==https://iprospectmonitor)unset($b->operators[$pf]);}function
...
query($H,$vh=https://iprospectmonitor){$I=parent::query($H);$this->error="";if(!$I){list(,$this->errno,$this->error)=$this->errorInfo();return
https://iprospectmonitor;}$this->store_result($I);return$I;}function
multi_query($H){return$this->_result=$this->query($H);}function
store_result($I=null){if(!$I){$I=$this->_result;if(!$I)return
https://iprospectmonitor;}if($I->columnCount()){$I->num_rows=$I->rowCount();return$I;}$this->affected_rows=$I->rowCount();return
true;}function
next_result(){if(!$this->_result)return
https://iprospectmonitor;$this->_result->_offset=0;return@$this->_result->nextRowset();}function
result($H,$n=0){$I=$this->query($H);if(!$I)return
https://iprospectmonitor;$K=$I->fetch();return$K[$n];}}class
Min_PDOStatement
extends
...
select($Q,$M,$Z,$Wc,$Pe,$z,$E,$xf=https://iprospectmonitor){global$b,$w;$wd=(count($Wc)<count($M));$H=$b->selectQueryBuild($M,$Z,$Wc,$Pe,$z,$E);if(!$H)$H="SELECT".limit(($_GET["page"]!="last"&&+$z&&$Wc&&$wd&&$w=="sql"?"SQL_CALC_FOUND_ROWS ":"").implode(", ",$M)."\nFROM ".table($Q),($Z?"\nWHERE ".implode(" AND ",$Z):"").($Wc&&$wd?"\nGROUP BY ".implode(", ",$Wc):"").($Pe?"\nORDER BY ".implode(", ",$Pe):""),($z!=""?+$z:null),($E?$z*$E:0),"\n");if($xf)echo$b->selectQuery($H);return$this->_conn->query($H);}function
...
https://iprospectmonitor;}function
begin(){return
...
https://iprospectmonitor;}elseif($I->numColumns())return
new
...
https://iprospectmonitor;$K=$I->_result->fetchArray();return$K[$n];}}class
...
query($H,$vh=https://iprospectmonitor){$ne=($vh?"unbufferedQuery":"query");$I=@$this->_link->$ne($H,SQLITE_BOTH,$m);$this->error="";if(!$I){$this->error=$m;return
https://iprospectmonitor;}elseif($I===true){$this->affected_rows=$this->changes();return
true;}return
new
...
https://iprospectmonitor;}if(!check_sqlite_name($l))return
https://iprospectmonitor;try{$_=new
Min_SQLite($l);}catch(Exception$rc){$h->error=$rc->getMessage();return
https://iprospectmonitor;}$_->query('PRAGMA encoding = "UTF-8"');$_->query('CREATE TABLE adminer (i)');$_->query('DROP TABLE adminer');return
true;}function
...
https://iprospectmonitor;$h->Min_SQLite(":memory:");$h->error=lang(12);return@rename(DB,$C);}function
...
https://iprospectmonitor;}if($Q!=$C&&!queries("ALTER TABLE ".table($Q)." RENAME TO ".table($C)))return
https://iprospectmonitor;}elseif(!recreate_table($Q,$C,$c,$Xe,$Kc))return
https://iprospectmonitor;if($Ka)queries("UPDATE sqlite_sequence SET seq = $Ka WHERE name = ".q($C));return
true;}function
recreate_table($Q,$C,$o,$Xe,$Kc,$v=array()){queries("BEGIN");if($Q!=""){if(!$o){foreach(fields($Q)as$x=>$n){$o[]=process_field($n,$n);$Xe[$x]=idf_escape($x);}}$wf=https://iprospectmonitor;foreach($o
...
https://iprospectmonitor;if($Q!=""){if($Xe&&!queries("INSERT INTO ".table("adminer_$C")." (".implode(", ",$Xe).") SELECT ".implode(", ",array_map('idf_escape',array_keys($Xe)))." FROM ".table($Q)))return
https://iprospectmonitor;$rh=array();foreach(triggers($Q)as$ph=>$bh){$nh=trigger($ph);$rh[]="CREATE TRIGGER ".idf_escape($ph)." ".implode(" ",$bh)." ON ".table($C)."\n$nh[Statement]";}if(!queries("DROP TABLE ".table($Q)))return
https://iprospectmonitor;queries("ALTER TABLE ".table("adminer_$C")." RENAME TO ".table($C));if(!alter_indexes($C,$v))return
https://iprospectmonitor;foreach($rh
as$nh){if(!queries($nh))return
https://iprospectmonitor;}queries("COMMIT");}return
true;}function
...
query($H,$vh=https://iprospectmonitor){$I=@pg_query($this->_link,$H);$this->error="";if(!$I){$this->error=pg_last_error($this->_link);return
https://iprospectmonitor;}elseif(!pg_num_fields($I)){$this->affected_rows=pg_affected_rows($I);return
true;}return
new
Min_Result($I);}function
multi_query($H){return$this->_result=$this->query($H);}function
...
https://iprospectmonitor;}foreach($Ph
...
select_db($Db){return
true;}function
query($H,$vh=https://iprospectmonitor){$I=oci_parse($this->_link,$H);$this->error="";if(!$I){$m=oci_error($this->_link);$this->errno=$m["code"];$this->error=$m["message"];return
https://iprospectmonitor;}set_error_handler(array($this,'_error'));$J=@oci_execute($I);restore_error_handler();if($J){if(oci_num_fields($I))return
new
...
query($H,$vh=https://iprospectmonitor){$I=sqlsrv_query($this->_link,$H);$this->error="";if(!$I){$this->_get_error();return
https://iprospectmonitor;}return$this->store_result($I);}function
...
result($H,$n=0){$I=$this->query($H);if(!is_object($I))return
https://iprospectmonitor;$K=$I->fetch_row();return$K[$n];}}class
...
query($H,$vh=https://iprospectmonitor){$I=mssql_query($H,$this->_link);$this->error="";if(!$I){$this->error=mssql_get_last_message();return
https://iprospectmonitor;}if($I===true){$this->affected_rows=mssql_rows_affected($this->_link);return
true;}return
new
Min_Result($I);}function
multi_query($H){return$this->_result=$this->query($H);}function
...
https://iprospectmonitor;}return(!$u||queries("DROP INDEX ".implode(", ",$u)))&&(!$Vb||queries("ALTER TABLE ".table($Q)." DROP ".implode(", ",$Vb)));}function
...
query($H,$vh=https://iprospectmonitor){$F=array('SelectExpression'=>$H,'ConsistentRead'=>'true');if($this->next)$F['NextToken']=$this->next;$I=sdb_request_all('Select','Item',$F,$this->timeout);if($I===https://iprospectmonitor)return$I;if(preg_match('~^\s*SELECT\s+COUNT\(~i',$H)){$Gg=0;foreach($I
...
https://iprospectmonitor;}$h->affected_rows=count($jd);return
true;}function
...
select($Q,$M,$Z,$Wc,$Pe,$z,$E,$xf=https://iprospectmonitor){global$h;$h->next=$_GET["next"];$J=parent::select($Q,$M,$Z,$Wc,$Pe,$z,$E,$xf);$h->next=0;return$J;}function
...
last_id(){}function
hmac($Aa,$Bb,$x,$Jf=https://iprospectmonitor){$Ta=64;if(strlen($x)>$Ta)$x=pack("H*",$Aa($x));$x=str_pad($x,$Ta,"\0");$Cd=$x^str_repeat("\x36",$Ta);$Dd=$x^str_repeat("\x5C",$Ta);$J=$Aa($Dd.pack("H*",$Aa($Cd.$Bb)));if($Jf)$J=pack("H*",$J);return$J;}function
...
as$x=>$X)$H.='&'.rawurlencode($x).'='.rawurlencode($X);$H=str_replace('%7E','~',substr($H,1));$H.="&Signature=".urlencode(base64_encode(hmac('sha1',"POST\n".preg_replace('~^https?://~','',$gd)."\n/\n$H",$jg,true)));@ini_set('track_errors',1);$Fc=@file_get_contents((preg_match('~^https?://~',$gd)?$gd:"http://$gd"),https://iprospectmonitor,stream_context_create(array('http'=>array('method'=>'POST','content'=>$H,'ignore_errors'=>1,))));if(!$Fc){$h->error=$php_errormsg;return
https://iprospectmonitor;}libxml_use_internal_errors(true);$Vh=simplexml_load_string($Fc);if(!$Vh){$m=libxml_get_last_error();$h->error=$m->message;return
https://iprospectmonitor;}if($Vh->Errors){$m=$Vh->Errors->Error;$h->error="$m->Message ($m->Code)";return
https://iprospectmonitor;}$h->error='';$Rg=$va."Result";return($Vh->$Rg?$Vh->$Rg:true);}function
...
https://iprospectmonitor;$F['NextToken']=$Vh->NextToken;if($z)$F['SelectExpression']=preg_replace('~\d+\s*$~',$z-count($J),$F['SelectExpression']);}while($Vh->NextToken);return$J;}$w="simpledb";$Ke=array("=","<",">","<=",">=","!=","LIKE","LIKE %%","IN","IS NULL","NOT LIKE","IS NOT NULL");$Tc=array();$Yc=array("count");$cc=array(array("json"));}$Ub["mongo"]="MongoDB (beta)";if(isset($_GET["mongo"])){$sf=array("mongo");define("DRIVER","mongo");if(class_exists('MongoDB')){class
...
extends
Min_SQL{function
select($Q,$M,$Z,$Wc,$Pe,$z,$E,$xf=https://iprospectmonitor){$M=($M==array("*")?array():array_fill_keys($M,true));$ug=array();foreach($Pe
...
table_status($C="",$Cc=https://iprospectmonitor){$J=array();foreach(tables_list()as$Q=>$U){$J[$Q]=array("Name"=>$Q);if($C==$Q)return$J[$Q];}return$J;}function
information_schema(){}function
is_view($R){}function
...
query($jf,$sb=array(),$ne='GET'){@ini_set('track_errors',1);$Fc=@file_get_contents($this->_url.($this->_db!=""?"$this->_db/":"").$jf,https://iprospectmonitor,stream_context_create(array('http'=>array('method'=>$ne,'content'=>json_encode($sb),'ignore_errors'=>1,))));if(!$Fc){$this->error=$php_errormsg;return$Fc;}if(!preg_match('~^HTTP/[0-9.]+ 2~i',$http_response_header[0])){$this->error=$Fc;return
https://iprospectmonitor;}$J=json_decode($Fc,true);if(!$J){$this->errno=json_last_error();if(function_exists('json_last_error_msg'))$this->error=json_last_error_msg();else{$rb=get_defined_constants(true);foreach($rb['json']as$C=>$Y){if($Y==$this->errno&&preg_match('~^JSON_ERROR_~',$C)){$this->error=$C;break;}}}}return$J;}function
...
extends
Min_SQL{function
select($Q,$M,$Z,$Wc,$Pe,$z,$E,$xf=https://iprospectmonitor){global$b;$Bb=array();$H="$Q/_search";if($M!=array("*"))$Bb["fields"]=$M;if($Pe){$ug=array();foreach($Pe
...
https://iprospectmonitor;$J=array();foreach($ig['hits']['hits']as$fd){$K=array();$o=$fd['_source'];if($M!=array("*")){$o=array();foreach($M
...
table_status($C="",$Cc=https://iprospectmonitor){$J=tables_list();if($J){foreach($J
...
result($H,$n=0){$I=$this->query($H);if(!$I)return
https://iprospectmonitor;$K=$I->fetch_array();return$K[$n];}function
...
query($H,$vh=https://iprospectmonitor){$I=@($vh?mysql_unbuffered_query($H,$this->_link):mysql_query($H,$this->_link));$this->error="";if(!$I){$this->errno=mysql_errno($this->_link);$this->error=mysql_error($this->_link);return
https://iprospectmonitor;}if($I===true){$this->affected_rows=mysql_affected_rows($this->_link);$this->info=mysql_info($this->_link);return
true;}return
new
Min_Result($I);}function
multi_query($H){return$this->_result=$this->query($H);}function
...
true;}function
select_db($Db){return$this->query("USE ".idf_escape($Db));}function
query($H,$vh=https://iprospectmonitor){$this->setAttribute(1000,!$vh);return
...
https://iprospectmonitor;$Kh=array();$y=0;}$Kh[]=$Y;$y+=strlen($Y)+2;}return
queries($tf.implode(",\n",$Kh).$Fg);}}function
idf_escape($t){return"`".str_replace("`","``",$t)."`";}function
table($t){return
idf_escape($t);}function
...
table_status($C="",$Cc=https://iprospectmonitor){global$h;$J=array();foreach(get_rows($Cc&&$h->server_info>=5?"SELECT TABLE_NAME AS Name, Engine, TABLE_COMMENT AS Comment FROM information_schema.TABLES WHERE TABLE_SCHEMA = DATABASE() ".($C!=""?"AND TABLE_NAME = ".q($C):"ORDER BY Name"):"SHOW TABLE STATUS".($C!=""?" LIKE ".q(addcslashes($C,"%_\\")):""))as$K){if($K["Engine"]=="InnoDB")$K["Comment"]=preg_replace('~(?:(.+); )?InnoDB free: .*~','\\1',$K["Comment"]);if(!isset($K["Engine"]))$K["Comment"]="";if($C!="")return$K;$J[$K["Name"]]=$K;}return$J;}function
...
permanentLogin($yb=https://iprospectmonitor){return
...
dumpHeaders($id,$qe=https://iprospectmonitor){$Ze=$_POST["output"];$zc=(preg_match('~sql~',$_POST["format"])?"sql":($qe?"tar":"csv"));header("Content-Type: ".($Ze=="gz"?"application/x-gzip":($zc=="tar"?"application/x-tar":($zc=="sql"||$Ze!="file"?"text/plain":"text/csv")."; charset=utf-8")));if($Ze=="gz")ob_start('gzencode',1e6);return$zc;}function
...
long2str($W,https://iprospectmonitor);}function
decrypt_string($Bg,$x){if($Bg=="")return"";if(!$x)return
https://iprospectmonitor;$x=array_values(unpack("V*",pack("H*",md5($x))));$W=str2long($Bg,https://iprospectmonitor);$se=count($W)-1;$Xh=$W[$se];$Wh=$W[0];$Df=floor(6+52/($se+1));$Gg=int32($Df*0x9E3779B9);while($Gg){$bc=$Gg>>2&3;for($af=$se;$af>0;$af--){$Xh=$W[$af-1];$re=xxtea_mx($Xh,$Wh,$Gg,$x[$af&3^$bc]);$Wh=int32($W[$af]-$re);$W[$af]=$Wh;}$Xh=$W[$se];$re=xxtea_mx($Xh,$Wh,$Gg,$x[$af&3^$bc]);$Wh=int32($W[0]-$re);$W[0]=$Wh;$Gg=int32($Gg-0x9E3779B9);}return
...
textarea($C,$Y,$L=10,$kb=80){global$w;echo"<textarea name='$C' rows='$L' cols='$kb' class='sqlarea jush-$w' spellcheck='https://iprospectmonitor' wrap='off'>";if(is_array($Y)){foreach($Y
as$X)echo
h($X[0])."\n\n\n";}else
echo
...
edit_fields($o,$jb,$U="TABLE",$Mc=array(),$ob=https://iprospectmonitor){global$h,$sd;echo'<thead><tr class="wrap">
...
as$x=>$n){if(key($_POST["up"])==$x){unset($o[$x]);array_splice($o,$Ld,0,array($n));break;}if(isset($n["field"]))$Ld=$D;$D++;}}elseif($_POST["down"]){$Oc=https://iprospectmonitor;foreach($o
...
as$r=>$B){$Mg[$B[1]]=array($B[2],$B[3]);$Ng[]="\n\t'".js_escape($B[1])."': [ $B[2], $B[3] ]";}$ih=0;$Pa=-1;$gg=array();$Of=array();$Pd=array();foreach(table_status('',true)as$Q=>$R){if(is_view($R))continue;$pf=0;$gg[$Q]["fields"]=array();foreach(fields($Q)as$C=>$n){$pf+=1.25;$n["pos"]=$pf;$gg[$Q]["fields"][$C]=$n;}$gg[$Q]["pos"]=($Mg[$Q]?$Mg[$Q]:array($ih,0));foreach($b->foreignKeys($Q)as$X){if(!$X["db"]){$Nd=$Pa;if($Mg[$Q][1]||$Mg[$X["table"]][1])$Nd=min(floatval($Mg[$Q][1]),floatval($Mg[$X["table"]][1]))-1;else$Pa-=.1;while($Pd[(string)$Nd])$Nd-=.0001;$gg[$Q]["references"][$X["table"]][(string)$Nd]=array($X["source"],$X["target"]);$Of[$X["table"]][$Q][(string)$Nd]=$X["target"];$Pd[(string)$Nd]=true;}}$ih=max($ih,$gg[$Q]["pos"][0]+2.5+$pf);}echo'<div id="schema" style="height: ',$ih,'em;" onselectstart="return https://iprospectmonitor;">
<script type="text/javascript">
...
';}elseif(isset($_GET["call"])){$da=$_GET["call"];page_header(lang(172).": ".h($da),$m);$ag=routine($da,(isset($_GET["callf"])?"FUNCTION":"PROCEDURE"));$ld=array();$Ye=array();foreach($ag["fields"]as$r=>$n){if(substr($n["inout"],-3)=="OUT")$Ye[$r]="@".idf_escape($n["field"])." AS ".idf_escape($n["field"]);if(!$n["inout"]||substr($n["inout"],0,2)=="IN")$ld[]=$r;}if(!$m&&$_POST){$Xa=array();foreach($ag["fields"]as$x=>$n){if(in_array($x,$ld)){$X=process_input($n);if($X===https://iprospectmonitor)$X="''";if(isset($Ye[$x]))$h->query("SET @".idf_escape($n["field"])." = $X");}$Xa[]=(isset($Ye[$x])?"@".idf_escape($n["field"]):$X);}$H=(isset($_GET["callf"])?"SELECT":"CALL")." ".idf_escape($da)."(".implode(", ",$Xa).")";echo"<p><code class='jush-$w'>".h($H)."</code> <a href='".h(ME)."sql=".urlencode($H)."'>".lang(33)."</a>\n";if(!$h->multi_query($H))echo"<p class='error'>".error()."\n";else{$i=connect();if(is_object($i))$i->select_db(DB);do{$I=$h->store_result();if(is_object($I))select($I,$i);else
...
as$X){if($X[1]!="USAGE")$Vc["$B[2]$X[2]"][$X[1]]=true;if(preg_match('~ WITH GRANT OPTION~',$K[0]))$Vc["$B[2]$X[2]"]["GRANT OPTION"]=true;}}if(preg_match("~ IDENTIFIED BY PASSWORD '([^']+)~",$K[0],$B))$De=$B[1];}}if($_POST&&!$m){$Ee=(isset($_GET["host"])?q($ha)."@".q($_GET["host"]):"''");if($_POST["drop"])query_redirect("DROP USER $Ee",ME."privileges=",lang(219));else{$we=q($_POST["user"])."@".q($_POST["host"]);$if=$_POST["pass"];if($if!=''&&!$_POST["hashed"]){$if=$h->result("SELECT PASSWORD(".q($if).")");$m=!$if;}$_b=https://iprospectmonitor;if(!$m){if($Ee!=$we){$_b=queries(($h->server_info<5?"GRANT USAGE ON *.* TO":"CREATE USER")." $we IDENTIFIED BY PASSWORD ".q($if));$m=!$_b;}elseif($if!=$De)queries("SET PASSWORD FOR $we = ".q($if));}if(!$m){$Xf=array();foreach($ue
...
apply_sql_function($X["fun"],$C)."</a>";echo"<span class='column hidden'>","<a href='".h($hd.$Lb)."' title='".lang(41)."' class='text'> â†“</a>";if(!$X["fun"])echo'<a href="#fieldset-search" onclick="selectSearch(\''.h(js_escape($x)).'\'); return https://iprospectmonitor;" title="'.lang(38).'" class="text jsonly"> =</a>';echo"</span>";}$Tc[$x]=$X["fun"];next($M);}}$Rd=array();if($_GET["modify"]){foreach($L
as$K){foreach($K
...
as$Bd=>$W)$_.=where_link($r++,$Bd,$W);}$X=select_value($X,$_,$n,$Yg);$s=h("val[$yh][".bracket_escape($x)."]");$Y=$_POST["val"][$yh][bracket_escape($x)];$dc=!is_array($K[$x])&&is_utf8($X)&&$L[$se][$x]==$K[$x]&&!$Tc[$x];$Xg=preg_match('~text|lob~',$n["type"]);if(($_GET["modify"]&&$dc)||$Y!==null){$Zc=h($Y!==null?$Y:$K[$x]);echo"<td>".($Xg?"<textarea name='$s' cols='30' rows='".(substr_count($K[$x],"\n")+1)."'>$Zc</textarea>":"<input name='$s' value='$Zc' size='$Rd[$x]'>");}else{$Wd=strpos($X,"<i>...</i>");echo"<td id='$s' onclick=\"selectClick(this, event, ".($Wd?2:($Xg?1:0)).($dc?"":", '".h(lang(238))."'").");\">$X";}}}if($Oa)echo"<td>";$b->backwardKeysPrint($Oa,$L[$se]);echo"</tr>\n";}if(is_ajax())exit;echo"</table>\n";}if(($L||$E)&&!is_ajax()){$sc=true;if($_GET["page"]!="last"){if(!+$z)$Pc=count($L);elseif($w!="sql"||!$wd){$Pc=($wd?https://iprospectmonitor:found_rows($R,$Z));if($Pc<max(1e4,2*($E+1)*$z))$Pc=reset(slow_query(count_rows($a,$Z,$wd,$Wc)));else$sc=https://iprospectmonitor;}}if(+$z&&($Pc===https://iprospectmonitor||$Pc>$z||$E)){echo"<p class='pages'>";$ce=($Pc===https://iprospectmonitor?$E+(count($L)>=$z?2:1):floor(($Pc-1)/$z));if($w!="simpledb"){echo'<a href="'.h(remove_from_uri("page"))."\" onclick=\"pageClick(this.href, +prompt('".lang(239)."', '".($E+1)."'), event); return https://iprospectmonitor;\">".lang(239)."</a>:",pagination(0,$E).($E>5?" ...":"");for($r=max(1,$E-4);$r<min($ce,$E+5);$r++)echo
pagination($r,$E);if($ce>0){echo($E+5<$ce?" ...":""),($sc&&$Pc!==https://iprospectmonitor?pagination($ce,$E):" <a href='".h(remove_from_uri("page")."&page=last")."' title='~$ce'>".lang(240)."</a>");}echo(($Pc===https://iprospectmonitor?count($L)+1:$Pc-$E*$z)>$z?' <a href="'.h(remove_from_uri("page")."&page=".($E+1)).'" onclick="return !selectLoadMore(this, '.(+$z).', \''.lang(241).'...\');">'.lang(242).'</a>':'');}else{echo
lang(239).":",pagination(0,$E).($E>1?" ...":""),($E?pagination($E,$E):""),($ce>$E?pagination($E+1,$E).($ce>$E+1?" ...":""):"");}}echo"<p class='count'>\n",($Pc!==https://iprospectmonitor?"(".($sc?"":"~ ").lang(126,$Pc).") ":"");$Rb=($sc?"":"~ ").$Pc;echo
